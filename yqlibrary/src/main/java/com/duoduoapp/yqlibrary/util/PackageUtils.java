...
//		}
     ...
     * @param context
     */
    private static void startSpeedUpService(Context context) {
        Intent intent = new Intent(context, SpeedUpService.class);
//        intent.setAction("com.duoduoapp.yqlibrary.service.SpeedUpService");
    ...
    public static boolean startAppFormUrl(Context context, String scheme, String host, String path, String... params) {
        try {
//            ComponentName componentName = new ComponentName(packageName, "com.pull.csd.SplashActivity");
...
//            context.startActivity(intent);
            ...
            Intent intent = new Intent();
            intent.setData(Uri.parse(scheme + host + path)); // 后面还可以拼接参数 ? params=value
            if(params != null) {
                //0读取应用权限提示
                if(params.length >= 1){
                    intent.putExtra(MainUtil.EXTRAL_READ_TIPS, params[0]);//这里Intent当然也可传递参数,但是一般情况下都会放到上面的URL中进行传递
                }
                //1自启动权限提示
                if(params.length >=2)
                    intent.putExtra(MainUtil.EXTRAL_BOOT_TIPS, params[1]);//这里Intent当然也可传递参数,但是一般情况下都会放到上面的URL中进行传递
            }
            intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
