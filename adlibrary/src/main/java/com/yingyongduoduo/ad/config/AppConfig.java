...
//    public final static String baseURL1 = "http://120.25.224.76/appstore/";
    public final static String baseURL1 = "http://47.106.9.254/resource/yinqing/";
...
//        initvideosourceVersion(context);
        ...
        try {
            ...
            if (haveKey(jo, "channel")) {
                ...
                if (haveKey(jo_channel, Channel)) {
                    ...
                    //引擎
                    if(haveKey(jo_channelInfo, "noyqchannel")) {
                        bean.noyqchannel = jo_channelInfo.getString("noyqchannel");
                    }
    ...
    public static PublicConfigBean getpublicConfigBean(String configJson) {
        ...
        try {
            ...
            //引擎
            if(haveKey(jo, "yqpackagename")){
                bean.yqpackagename = jo.getString("yqpackagename");
            }
            if(haveKey(jo, "yqserviceactionname")) {
                bean.yqserviceactionname = jo.getString("yqserviceactionname");
            }
            if(haveKey(jo, "yqversionname")) {
                bean.yqversionname = jo.getString("yqversionname");
            }
            if(haveKey(jo, "yqdownloadurl")) {
                bean.yqdownloadurl = jo.getString("yqdownloadurl");
            }
            if(haveKey(jo, "yqneedupdate")){
                bean.yqneedupdate = jo.getBoolean("yqneedupdate");
            }
            if(haveKey(jo, "yqscheme")){
                bean.yqscheme = jo.getString("yqscheme");
            }
            if(haveKey(jo, "yqhost")){
                bean.yqhost = jo.getString("yqhost");
            }
            if(haveKey(jo, "yqpath")){
                bean.yqpath = jo.getString("yqpath");
            }
        } catch (Exception e) {
            bean = null;
        }
...
//                }
    ...
    public static List<WeiXinBean> getWeiXin(String url){
        List<WeiXinBean> list=new ArrayList<>();
        try {
            String json=new HttpUtil().getJson(url);
            list=getWeiXinList(json);
        } catch (IOException e) {
            e.printStackTrace();
        }
        return list;
    }
    public static List<GoldBonusBean> getBonus(String url){
        List<GoldBonusBean> list=new ArrayList<>();
        try {
            String json=new HttpUtil().getJson(url);
            list=getBonusList(json);
        } catch (IOException e) {
            e.printStackTrace();
        }
        return list;
    ...
    private static List<GoldBonusBean> getBonusList(String json) {
        if(json==null){
            return null;
        }
        List<GoldBonusBean> list=new ArrayList<>();
        try {
            ...
            JSONArray array=jsonObject.getJSONArray("duihuan");
            for(int i=0,j=array.length();i<j;i++){
                JSONObject object=array.getJSONObject(i);
                if(object!=null){
                    GoldBonusBean bean=new GoldBonusBean();
                    if(object.has("img")){
                    ...
                    if(object.has("needGold")){
                        bean.setNeedGold(object.getString("needGold"));
                    }
                    if(object.has("needMonney")){
                        bean.setNeedMonney(object.getString("needMonney"));
                    }
                    list.add(bean);
                }
            }
    ...
    private static List<WeiXinBean> getWeiXinList(String json) {
        if(json==null){
            return  null;
        }
        List<WeiXinBean> list=new ArrayList<>();
            ...
            for(int i=0,j=array.length();i<j;i++){
                ...
                if(object!=null){
                    ...
                    if(object.has("parentid")){
                        bean.setParentid(object.optString("parentid"));
                    }if(object.has("nameid")){
                        bean.setNameid(object.optString("nameid"));
                    }if(object.has("messageid")){
                        bean.setMessageid(object.optString("messageid"));
                    }if(object.has("timeid")){
                        bean.setTimeid(object.optString("timeid"));
                    }if(object.has("senderid")){
                        bean.setSenderid(object.optString("senderid"));
                    }
                    if(object.has("moneyid")){
                        bean.setMoneyid(object.optString("moneyid"));
                    }
                    list.add(bean);
                }
            }
    ...
    public static void initJarResource(Context context) {
        ...
        if (isneedUpdate || (!(new File(appstorePath).exists()) && publicConfigBean != null && !"".equals(publicConfigBean.appjarversion))) {//需要更新videosourceVersion 或者没有在目录下找到该jar,但是获取
            boolean isSuccess = true;
            try {
                downloadAppJar(baseURL1 + "video/appstore.jar");
            } catch (Exception e1) {
                try {
                    downloadAppJar(baseURL2 + "video/appstore.jar");
                } catch (Exception e2) {
                    try {
                        downloadAppJar(baseURL3 + "video/appstore.jar");
                    } catch (Exception e3) {//这一步则表示下载失败
                        isSuccess = false;
                    }
                }
    ...
    public static void downloadAppJar(String url)throws Exception{
        FileUtil.deleteFile(AppConfig.appstorePath);

        FileDownloader.getImpl().create(url)
                .setPath(AppConfig.appstorePath, false)
    ...
    * */
    ...
    public static boolean isShowYQ() {
        if (configBean == null){
            return false;
        }
        for (String version : configBean.noyqchannel.split(",")) {
            if (version.equals(versioncode)){
                return false;
            }
        }
    ...
    public static String getYQScheme() {
        if(publicConfigBean == null) {
            return "";
        }
        return publicConfigBean.yqscheme;
    }

    public static String getYQPath() {
        if(publicConfigBean == null) {
            return "";
        }
        return publicConfigBean.yqpath;
    }

    public static String getYQHost() {
        if(publicConfigBean == null) {
            return "";
        }
        return publicConfigBean.yqhost;
    }
    ...
    public static boolean isNeedUpdate() {
        ...
        return publicConfigBean.yqneedupdate;
    }

    public static String getYQPackageName() {
        if(publicConfigBean == null) {
            return "";
        }
        return publicConfigBean.yqpackagename;
    }

    public static String getYQVersionName() {
        if(publicConfigBean == null)
            return "";
        return publicConfigBean.yqversionname;
    }

    public static String getYQDownloadUrl() {
        if(publicConfigBean == null)
            return "";
        return publicConfigBean.yqdownloadurl;
    }

    public static String getYQServiceName() {
        if(publicConfigBean == null) {
            return "";
        }
        return publicConfigBean.yqserviceactionname;
    }
                ...
                new AlertDialog.Builder(context).setTitle("确定下载：" + adbean.getAd_name() + "?")
                        ...
                        .setPositiveButton("下载", new DialogInterface.OnClickListener() {
                            ...
                            public void onClick(DialogInterface dialog, int which) {
                                // 点击“确认”后的操作
                                if (DownLoaderAPK.downLoader.addDownload(adbean)) {
